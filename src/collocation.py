...
import math # For calculating logarithms
...
def main(file_dir, keyword, window_size):
	# One directory up from the corpus' directory - file is named after the keyword and window size
	outfile = os.path.join(os.path.split(file_dir)[0], f'{keyword}_{window_size}.csv')
	...
	# Note: file_dir is already converted to a Path in ArgumentParser
	for novel_path in file_dir.glob('*.txt'): 
